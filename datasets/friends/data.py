...
def encode(q, r, w2idx):
    ...
    # find max len of sentences in q and r
    maxq = max([len(qi) for qi in q])
    maxr = max([len(ri) for ri in r])


    for i in range(data_len):
        idx_q.append(encode_seq(q[i], w2idx, maxq))
        idx_r.append(encode_seq(r[i], w2idx, maxr))
...
'''
def encode_seq(seq, lookup, maxlen):
    indices = []
    for word in seq:
        if word in lookup:
